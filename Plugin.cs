...
namespace OverlayHUD
{
    ...
    public sealed class Plugin : BaseUnityPlugin
    {
        ...
        private void Update()
        {
            ...
            // БОЛЬШЕ НИКАКОЙ ЗАВИСИМОСТИ ОТ ФОКУСА ОКНА ИГРЫ

            // АНТИ-МЕРЦАНИЕ КУРСОРОВ
            ...
            if (pendingCursorState != wasCursorVisible && Time.unscaledTime >= cursorStateChangeTime)
            {
                ...
                StartCoroutine(PostCursorState(wasCursorVisible));
            }
        ...
        private IEnumerator ActivateGameplayAfterLevelChange(string reason)
        {
            for (int attempt = 1; attempt <= 10; attempt++)
            {
                ...
                if (HandleGameplayDetected(reason + " attempt " + attempt)) { pendingGameplayActivation = null; yield break; }
                if (IsNonGameplayContext()) break;
            }
        ...
        private void SyncMapValueIfChanged()
        {
            ...
            StartCoroutine(PostMapValue(val, init, lost, goal));
        }
        ...
        private void RevealAllKnownEnemies(List<ResolvedEnemyCandidate> enemies)
        {
            foreach (var res in enemies)
            {
                ...
                StartCoroutine(PostSeenMonster(res.MonsterName, iId));
            }
        }
        ...
        private static bool TryGetEnemyRespawnStatus(EnemyCandidate candidate, out bool alive, out float remaining)
        {
            ...
            if (instance != null && candidate.Root != null)
            {
                ...
                if (!alive)
                {
                    ...
                    else if (instance.clientSimulatedTimers.TryGetValue(id, out float sim) && sim > 0f)
                    {
                        remaining = sim;
                    }
                    else
                    {
                        instance.clientSimulatedTimers[id] = 60f;
                        remaining = 60f;
                    }
                }
                else
        ...
        {
            if (gameplayActive && enemyParent != null) StartCoroutine(SyncEnemyParentStatusChangedNextFrame(enemyParent));
        }
        ...
        private void SyncEnemyParentTimerChanged(Component enemyParent)
        {
            ...
            StartCoroutine(PostMonsterStatuses("{\"statuses\":[{\"id\":" + iId + ",\"alive\":" + (a ? "true" : "false") + ",\"respawnRemaining\":" + rt + "}]}"));
        }
        ...
        private void SyncPlayerUpgradesIfChanged(HashSet<string> onlyKeys = null)
        {
            ...
            StartCoroutine(PostPlayerUpgrades(json.ToString(), playerCount));
        }
